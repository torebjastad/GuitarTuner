...
| `STABIL_TERSKEL` | 12 | Minimum confirmed pitch readings before accepting a result |
| `SAMLETID_MS` | 2500 | Collection window duration (2.5 s for better reproducibility) |
| `MAKS_VENTETID_MS` | 12 000 | Per-key timeout before giving up |
...
| `RMS_FALLOFF` | 0.10 | Early-exit trigger: if RMS drops below 10 % of peak |
| `ONSET_STILLE_RMS` | 0.003 | RMS must fall below this to be considered "silent" |
| `ONSET_ANSLAG_RMS` | 0.004 | RMS must rise above this to detect a new key press |
...
| `ONSET_TRANSIENT_MS` | 60 | Wait after onset before sampling (skips noisy attack transient) |
...
6. **Normal exit:** If `samletid ≥ SAMLETID_MS` AND `avlesninger.length ≥ STABIL_TERSKEL` → confirm after 2.5 seconds.
...
Applied only for notes ≥ 80 Hz (not bass — see below), and only when the refinement passes a reliability gate.
...
**Reliability gate:** The DTFT result is only accepted if the correction from the median is ≤ 3 cents. If the DTFT latches onto a sideband or harmonic and would shift the result by more than 3 cents, the median value is kept instead. This prevents DTFT from worsening an already-good estimate.
...
3. Apply **DTFT refinement** if `forventetHz ≥ 80 Hz` and the refinement passes the reliability gate (correction ≤ 3 cents from median).
